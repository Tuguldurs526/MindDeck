...
export async function overview(req: Request, res: Response) {
  ...
  const reviewedToday = await ReviewState.countDocuments({
    user: userId,
    lastReviewedAt: { $gte: start, $lte: end },
  });

  const todays = await ReviewState.find(
    { user: userId, lastReviewedAt: { $gte: start, $lte: end } },
    { lastRating: 1 }
  );
  const ok = todays.filter((t: any) => t.lastRating && t.lastRating !== "again").length;
